...
if(isset($_GET['id'])&& isset($_SESSION["email"])){
    ...
    $udn = $row["date_naiss"];
    $udi = $row["date_insc"];
    $age = date_diff(date_create($udn), date_create('today'))->y; // l'age
...
.val {
    background-color: rgb(94,153,79);
    border: none;
    border-radius: 5px;
    cursor: pointer;
    color: #ffffff;
...
<body>
    ...
    <p> <strong> Attestaion : </strong> <a href="<?php echo $attpath?>" target="new">cliquez ici</a> </p> 
        ...
        <br><br>
        <p>
        <button type="button" class="val"> <a class="val" href='valider.php?id=<?php echo $uid ?>'>Valider</a></button>
        <button type="button" class="ref"  onclick="return confirm('Vous voulez vraiment refuser ce compte? Cette action est irréversible')"> <a class="ref" href='refuser.php?id=<?php echo $uid ?>'>Refuser</a></button>
        <p>
